        ...
        return (
            <footer className="footer container">
                ...
                <nav>
                    ...
                    <Link key={link_idx} className="subtle-link" to={safePrefix(_.get(link, 'url', null))}{...(_.get(link, 'new_window', null) ? ({target: '_blank', rel: 'noopener'}) : null)}>{_.get(link, 'label', null)}</Link>
                    ))}
                </nav>
                )}
