    ...
    render() {
        ...
        return (
            <section id={_.get(section, 'section_id', null)} className="posts">
                ...
                <Link key={post_idx} to={safePrefix(_.get(post, 'url', null))} className="article-teaser">
                    {_.get(post, 'frontmatter.thumb_img_path', null) && (<img className="thumbnail" src={safePrefix(_.get(post, 'frontmatter.thumb_img_path', null))} alt={_.get(post, 'frontmatter.title', null)}/>)}
                    <div className="copy">
